...
Each node needs to know about its peers. Using the local mailbox transport:

```bash
# Register the owner as a peer of the principal
...
# Register the worker as a peer of the owner
...
<Callout type="tip">
  Replace the `<...>` placeholders with the actual values from `identity show`.
</Callout>
